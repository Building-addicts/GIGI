...
extension GigiSpeechService: AVSpeechSynthesizerDelegate {
    nonisolated func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didStart utterance: AVSpeechUtterance) {
        GigiDebugLogger.log("GIGI Speech: didStart utterance.length=\(utterance.speechString.count)")
        Task { @MainActor in GigiAudioManager.shared.notifySpeakingStarted() }
    }
    nonisolated func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didFinish utterance: AVSpeechUtterance) {
        GigiDebugLogger.log("GIGI Speech: didFinish utterance.length=\(utterance.speechString.count)")
    ...
    nonisolated func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didCancel utterance: AVSpeechUtterance) {
        GigiDebugLogger.log("GIGI Speech: didCancel utterance.length=\(utterance.speechString.count)")
        Task { @MainActor in GigiAudioManager.shared.notifySpeakingFinished() }
    }
}
