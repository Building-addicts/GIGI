...
| 10 | Post-FM override tiers (CloudDowngrade, CompoundCommand, FactAssertion, ReminderUpgrade, CalendarEventUpgrade, MessageWithoutBody, UnresolvedContact, ClarificationDowngrade) | regex on the FM result | **Safety nets for known FM misroutes.** The small on-device FM ignores @Guide few-shots (verified 2026-05-24), so these deterministic gates are how reminder/fact/message-body corrections actually happen. Keep them. CalendarEventUpgrade (2026-05-25) forces a calendar-creation command to native create_event — overriding even a make_call mis-route when the title embeds 'call' — and asks for a title when absent. |
