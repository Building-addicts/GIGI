...
| **15** | Smart Action Loop — Plan / Confirm / Build / Learn | ~7-9h | 🚧 IN PROGRESS — Step 0 + Step 4 shipped (commit `9277001`, 2026-05-13); Steps 0.5 / 0.6 / 1 / 1D planned | Phase 2 ADR-0014 (commit `8a4f1eb`) | 15.A→15.B→**15.B.5**→**15.B.6**→15.C→15.D→15.E | **5-step user-driven decision tree** sopra `GigiShortcutRegistry`. Step 1 EXECUTE TRY (Layer A NLU + B registry + C semantic + D Apple FM dynamic tool). Step 2 PLAN — server split `POST /compose-shortcut/plan` (Claude only, returns `{planId, title, summary, actions, aliases, systemPurpose}`, plan held 5min TTL). Step 3 BUILD — `POST /compose-shortcut/build {planId}` consume plan + cherri compile + sign. Step 4 LEARN — auto-register in registry + reload semantic router + toast `"I learned 'X'. Next time you say 'Y' I'll run it directly."`. Step 5 RECOGNIZE — next utterance Step 1 matches, no card. **Step 0.5** (added 2026-05-13, +45min) friction-reduction: voce/chat YES/NO consent intercept in `GigiRequestRouter.route()` quando `gigi.shortcutProposal != nil` (pattern come `gigi.contactDisambiguation`); evita il tap su Build CTA. **Step 0.6** (added 2026-05-13, +30min): `UIDocumentInteractionController.presentOpenInMenu` con `uti="com.apple.shortcut"` sostituisce `UIActivityViewController`; filtra share sheet a sole app che possono aprire `.shortcut` → 1 tap invece della grid generica. iOS proposal card (`ShortcutProposalCard.swift`) con title + summary + numbered actions emoji + Build/Cancel CTAs + hint `"Or say 'yes' to build, 'no' to cancel"`. Tutti gli user-facing strings in inglese. ADR-0015 PROPOSED → Accepted. Layer 4 vecchio (pattern detection) spostato in GATE 15.5 Daydream. |
...
**Totale effort GATE 9-15.5**: ~96-126h (~14-16 giorni full-time, oppure 7-9 settimane part-time @ ~15h/settimana). GATE 15 aggiunge ~7-9h (con Steps 0.5 + 0.6 friction-reduction polish) e GATE 15.5 (deferred post-MVP) altri ~6-8h sopra il baseline.
...
- **2026-05-13 (GATE 15 Step 0+4 shipped + Steps 0.5/0.6 friction-reduction planned)** — GATE 15 Step 0 (server split `/plan` + `/build` + `/job` + plans Map 5min TTL + iOS `ShortcutProposalCard` + bridge async + `GigiSmartOrchestrator.shortcutProposal` state machine) and Step 4 (Learn Phase auto-register + reload semantic router + toast) **SHIPPED** in commit `9277001` (IPA `GIGI-gate15-learn-timing-9277001.ipa`). End-to-end loop verified: user says *"build me a shortcut that..."* → card → tap Build → cherri sign → share sheet → "Aggiungi" in Shortcuts.app → toast + auto-register ✅. User feedback: *"troppi tap manuali"*. Added **Step 0.5 Voice/Chat consent** (~45min): `GigiRequestRouter.route()` intercept on `GigiSmartOrchestrator.shared.shortcutProposal` non-nil with whole-word EN+IT YES/NO regex + ≤4 word guard. Added **Step 0.6 One-tap share sheet** (~30min): `UIDocumentInteractionController.presentOpenInMenu(uti: "com.apple.shortcut")` replaces `UIActivityViewController` → filters to Shortcuts.app only; new `ShortcutDocDelegate`; fallback to legacy `UIActivityViewController` if `presentOpenInMenu` returns false. Apple sandbox keeps final "Aggiungi comando rapido" tap non-negotiable. GATE 15.5 Daydream NOT modified (out of scope). Effort revised 6-8h → 7-9h. Status `📋 PLANNED` → `🚧 IN PROGRESS`. Sub-gate sequence: 15.A → 15.B → **15.B.5** → **15.B.6** → 15.C → 15.D → 15.E.
