...
| **15** | Smart Router Architecture — semantic embedding fast-path (anticipato PRE-GATE 10 per fixare mis-routing) | MVP shipped 2026-05-12 (~4h); phase 2 +6-10h | ✅ MVP SHIPPED (2026-05-12) — phase 2 PLANNED | nessuno (richiede solo GigiVectorStore + 22 tool registry) | 15.A MVP shipped, 15.B-D planned | **GigiSemanticRouter** (NLEmbedding fast-path + ADR-0012). Catalog 22 tool × 5-12 trigger phrases EN+IT, cosine similarity vDSP_dotpr ~3-5ms per query. Hook in `GigiRequestRouter.route()` PRIMA di Apple FM. Top-1 ≥0.55 + gap ≥0.05 → dispatch diretto; below threshold → fall through ad Apple FM (no regression). Sostituisce i regex intercept di GATE 9.A/9.C che non scalavano. Phase 2: 50-query eval set + threshold tuning. Phase 3: telemetry self-correction loop. Phase 4: Apple FM 2-stage fallback per band 0.40-0.55. |
