...
| **15** | Shortcut Intelligence — Proactive Intent Routing | ~4-6h | 📋 PLANNED (2026-05-13) | Phase 2 ADR-0014 (commit `8a4f1eb`) | 15.A→15.D | **4-layer intent routing pipeline** sopra `GigiShortcutRegistry`. Layer 1: auto-alias generation at compose time (5-10 aliases EN+IT + systemPurpose inferito via secondo Claude call in `ios-build-shortcut.js`). Layer 2: `GigiSemanticRouter.dynamicCatalog` carica gli alias come virtual intent `run_registered_shortcut:<name>`. Layer 3: `FMShortcutInvokeTool` Apple FM dynamic tool con description computed property che enumera Shortcut registrati. Layer 4: `GigiUsagePatterns.swift` proactive pattern detection (≥3 ripetizioni in 7 giorni → propone build). Chiude il loop matrioska compose → install → recognize → dispatch Tier 1 senza intervento utente. ADR-0015 PROPOSED → Accepted. |

**Totale effort GATE 9-15**: ~90-115h (~13-15 giorni full-time, oppure 6-8 settimane part-time @ ~15h/settimana). GATE 15 aggiunge ~4-6h sopra il baseline.
 ...
 Tool calling)      Ollama)
   ...
   └────────┬────────┘
        ...
        GATE 14 (Week 6 — Macro Engine + voice authoring — ADR-0011 Accepted)
            │
            ▼
        GATE 15 (Shortcut Intelligence Proactive Routing — ADR-0015 Accepted)
            │  (può anche essere fatto SUBITO dopo Phase 2 ADR-0014, non richiede GATE 9-14)
            ▼
        🚀 v1.1 GIGI capability-rich + 4-layer discovery + voice-authored macro automation + auto-recognized shortcuts
```
